	...
	public void checkEvent() {
		...
		if(canTouchEvent) {
			if(gp.currentMap.getId() == 0) {
				...
				if(hit(0,16,9,"down")) {gp.playSE(2);teleport(gp.maps[2],20,3);}
			}
				...
				if(hit(2,14,3,"up")) {gp.playSE(2);teleport(gp.maps[0],10,10); if(gp.s.chapter == 2 && gp.s.swh[6]) {gp.s.part = 2;}}
				if(hit(2,20,3,"up")) {illusion(gp.iTile[gp.currentMap.getId()][1]);}
				if(hit(2,20,2,"up")) {gp.playSE(2);teleport(gp.maps[0],16,9);}
				if(hit(2,14,4,"down")) {}
				if(hitRow(2,11,"down")) {
